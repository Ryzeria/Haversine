...
// Tambahkan basemap
var basemap_topo = L.tileLayer('https://server.arcgisonline.com/ArcGIS/rest/services/World_Topo_Map/MapServer/tile/{z}/{y}/{x}', {
    attribution: 'Tiles &copy; Esri &mdash; Esri, DeLorme, NAVTEQ, TomTom, Intermap, iPC, USGS, FAO, NPS, NRCAN, GeoBase, Kadaster NL, Ordnance Survey, Esri Japan, METI, Esri China (Hong Kong), dan GIS User Community',
    maxZoom: 16
}).addTo(map);

var Esri_PetaDunia = L.tileLayer('https://server.arcgisonline.com/ArcGIS/rest/services/NatGeo_World_Map/MapServer/tile/{z}/{y}/{x}', {
...
L.easyButton('<span>&#8505;</span>', function() {
    kotakInfo.style.display = (kotakInfo.style.display === 'none') ? 'block' : 'none';
}).addTo(map);

// Tampilkan koordinat saat mouse berada di atas peta
var tampilanKoordinat = document.getElementById('coordInfo');
map.on('mousemove', function(e) {
    tampilanKoordinat.innerHTML = 'Lintang: ' + e.latlng.lat.toFixed(5) + ', Bujur: ' + e.latlng.lng.toFixed(5);
});
